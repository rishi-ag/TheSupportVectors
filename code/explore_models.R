...
# Run this if want to unify soil to one feature and area to another
#soil<-unify(features,1:40,"soil_type_")
...
#Subsample to avoid computing all dataset, p= proportion to sample
sample<-createDataPartition(y=labels,p=0.1,list=FALSE)#sample preserving proportion of classes
...
# out: 74%, 90% for class 4

# Rforest
model3 <- train(labels~.,data=trainset,method="rf")
...
accuracy<-data.frame(tree=model1,lda=model2,rforest=model3)
...
accuracy<-foreach(mod = length(mod_names), 
